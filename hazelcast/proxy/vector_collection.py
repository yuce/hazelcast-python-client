...
class VectorCollection(Proxy["BlockingVectorCollection"]):
    ...
    def get(self, key: Any) -> Awaitable[Optional[Document]]:
    ...
    def set(self, key: Any, document: Document) -> Awaitable[None]:
    ...
    def put(self, key: Any, document: Document) -> Awaitable[Optional[Document]]:
    ...
    async def put_all(self, map: Dict[Any, Document]) -> None:
        ...
        for partition_id, entry_list in partition_map.items():
            ...
            task = loop.create_task(self._invoke_on_partition(request, partition_id))
            tasks.append(task)
    ...
    def remove(self, key: Any) -> Awaitable[Optional[Document]]:
    ...
    def delete(self, key: Any) -> Awaitable[None]:
        """Removes the mapping for a key from this VectorCollection if it is present
        (optional operation).
    ...
    def optimize(self, index_name: str = None) -> Awaitable[None]:
    ...
    def _set_internal(self, key: Any, document: Document) -> Awaitable[None]:
        try:
    ...
    def _delete_internal(self, key: Any) -> Awaitable[None]:
        key_data = self._to_data(key)
        request = vector_collection_delete_codec.encode_request(self.name, key_data)
        return self._invoke_on_key(request, key_data)

    def _remove_internal(self, key: Any) -> Awaitable[Optional[Document]]:
        def handler(message):
    ...
    def _put_internal(self, key: Any, document: Document) -> Awaitable[Optional[Document]]:
        def handler(message):
